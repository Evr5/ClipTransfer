...
debug:
	@CC=clang CXX=clang++ $(CMAKE) -S . -B $(BUILD_DIR) -G $(GENERATOR) -DCMAKE_BUILD_TYPE=Debug -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$(abspath $(OUTPUT_DIR)) -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
	@$(CMAKE) --build $(BUILD_DIR) -- -j$(CORES)

release:
	@CC=clang CXX=clang++ $(CMAKE) -S . -B $(BUILD_DIR) -G $(GENERATOR) -DCMAKE_BUILD_TYPE=Release -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$(abspath $(OUTPUT_DIR)) -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
	@$(CMAKE) --build $(BUILD_DIR) -- -j$(CORES)

static:
	@CC=clang CXX=clang++ $(CMAKE) -S . -B $(STATIC_BUILD_DIR) -G $(GENERATOR) -DCMAKE_BUILD_TYPE=Release -DBUILD_STATIC=ON -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$(abspath $(OUTPUT_DIR)) -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
